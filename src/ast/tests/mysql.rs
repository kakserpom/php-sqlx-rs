...
fn test_render_basic() {
    ...
    collapsed_eq!(&query, "SELECT * FROM users WHERE status = ? AND id = ?");
...
fn test_render_optional_skip() {
    ...
    collapsed_eq!(&query, "SELECT * FROM users WHERE id = ?");
...
fn test_pagination() {
    ...
    println!("sql = {sql:#?}");
    collapsed_eq!(&sql, "SELECT * FROM users age ORDER BY id LIMIT ? OFFSET ?");
    assert_eq!(
        values,
